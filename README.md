...
| 방식 | 조건 | 동작 |
|---|---|---|
| **구글 로그인 + 승인** | `config.json` 에 Firebase 설정이 있을 때 | 구글 로그인 → 이름 입력 → 관리자 승인 후에만 작성·조회 가능. 서식의 위원명은 승인된 이름이 자동으로 쓰입니다. 로그인 없이 쓰는 우회 경로는 없습니다 |
| 접속 코드 | Firebase 설정이 없을 때 | 관리 › 설정의 접속 코드 / 총괄 코드 / 관리 코드로 화면을 구분 |

로그인 방식에서는 개인 제출본을 **본인만** 수정할 수 있습니다. 마스터 데이터(과목·기준·설정) 변경과 회원 승인은 관리자만 가능합니다.
