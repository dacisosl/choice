...
| 화면 | 내용 |
|---|---|
| ① 개인서류 작성 | 과목·1/2/3순위 선택 → 서식1 점수표 자동 초안 → 셀 편집(배점 초과 경고, 합계 재계산) → 핵심의견 체크 후 의견 생성 → 개인 추천의견(서식3형) → 미리보기·제출·인쇄/PDF·JSON 내보내기 |
| ② 평가총괄표 작성 | 과목별 제출 현황(위원 3인 미만 경고) → 서식2 위원 열·출판사 행 자동, 총점·평균·순위(동점 처리) 자동 → 공식 서식3 (평균 순위 자동, 위원 의견 종합 생성) → 확정 시 과목 마감(개인 문서 수정 차단) |
| ⚙ 관리 | 과목·출판사(CSV 업로드), 평가기준(기본 템플릿/과목별 오버라이드, 합계 100 검증, 가격·재정 항목 삭제 잠금), 의견 선택지, 위원 명단, **회원 관리**(구글 로그인 승인·권한), **제출 관리**(제출본·총괄표 조회·제출취소·확정취소·삭제·내려받기), 설정(문체·목표 총점·AI 모델), 진행 현황, 백업/복원 |

## 접근 권한

| 대상 | 로그인 | 할 수 있는 일 |
|---|---|---|
| **일반 교사** | 필요 없음 | 개인서류 작성·제출·인쇄. 접속하면 익명 세션이 자동으로 만들어져 본인이 만든 문서만 열고 고칠 수 있습니다 |
| **총괄 작성 교사** | 구글 로그인 + 관리자 승인 | 위 항목 + 과목별 제출 현황 조회, 서식2 총괄표와 공식 서식3 작성·확정 |
| **관리자** | 구글 로그인 + 관리자 지정 | 위 항목 + 과목·출판사·평가기준·설정 변경, 회원 승인, 제출 관리 |

일반 교사의 문서는 그 브라우저의 익명 세션에 묶입니다. 문서 id를 브라우저에 보관해 두었다가 다시 열며, 다른 사람은 열 수 없습니다. 브라우저 데이터를 지우거나 다른 기기로 옮기면 작성 중이던 문서를 다시 열 수 없으니 **제출까지 한 자리에서 마치는 것**을 권합니다. 구글 로그인을 하면 익명 세션이 그 계정에 연결되어 작성 중이던 문서가 그대로 유지됩니다.

Firebase 설정이 없으면 로그인 기능이 꺼지고 관리 › 설정의 접속 코드 방식으로 동작합니다.
...
3. **로그인 방식 켜기** — 빌드 › Authentication › Sign-in method 에서 **Google** 과 **익명** 공급업체를 모두 사용 설정합니다. 익명은 일반 교사가 로그인 없이 제출할 때 쓰입니다.
